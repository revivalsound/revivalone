  ...
  return <>
    ...
    <section className="auth-form-side"><div className="auth-panel">
      ...
      <form className="auth-form" onSubmit={signIn}>
        ...
        <label><span className="auth-password-row">PASSWORD<button type="button" onClick={resetPassword}>Forgot password?</button></span><span className="auth-password-field"><input required type={showPassword ? "text" : "password"} autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Your password" /><button className={`auth-eye-button${showPassword ? " is-visible" : ""}`} type="button" aria-label={showPassword ? "Hide password" : "Show password"} aria-pressed={showPassword} onClick={() => setShowPassword((visible) => !visible)}><span className="auth-eye-icon" aria-hidden="true" /></button></span></label>
